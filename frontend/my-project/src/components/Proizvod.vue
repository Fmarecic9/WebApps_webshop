...
<div class="bg-white">
  <div class="pt-6">
    ...
    <!-- Image gallery -->
    <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8" v-if="proizvod">
    ...
    >
      <div class="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8" v-if="proizvod">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">{{proizvod.naziv}}</h1>
      </div>

      <!-- Options -->
      <div class="mt-4 lg:row-span-3 lg:mt-0" v-if="proizvod">
            ...
            <fieldset aria-label="Choose a color" class="mt-4" >
              <div class="flex items-center space-x-3">
                <!-- Active and Checked: "ring ring-offset-1" -->
                  <button v-for="boja in proizvod.dostupneBoje" :key="boja" @click="odabranaBoja = boja" :class="['px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200', { 'bg-indigo-600 text-white': odabrana_boja === boja, 'bg-white text-gray-900 border border-gray-300': odabrana_boja !== boja }]">
                    {{ boja }}
                  </button>
              </div>
            </fieldset>
          ...
          <div class="mt-10" >
            <div class="flex items-center justify-between">
              ...
              <!-- v-for direktiva za iscrtavanje veličina -->
                ...
                >
                <button name="size-choice" value="_" class="sr-only" @click="odabranaVelicina = velicina"/>
                  <span>{{ velicina }}</span>
      ...
      <div class="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pb-16 lg:pr-8 lg:pt-6">
        ...
        <div>
          ...
          <div class="space-y-6">
            <p class="text-base text-gray-900" v-if="proizvod">
              {{proizvod.opis}}
            </p>
          </div>
        </div>
        ...
        <div class="mt-10">
          ...
          <div class="mt-4" v-if="proizvod">
            <ul role="list" class="list-disc space-y-2 pl-4 text-sm" >
              <li class="text-gray-400" v-for="k in proizvod.karakteristike" :key="k">
        ...
        <div class="mt-10">
          ...
          <div class="mt-4 space-y-6">
            <p class="text-sm text-gray-600">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore</p>
          </div>
        </div>
      </div>
    </div>
...
const dodajUKosaricu = async () => {
  if (odabranaBoja && odabranaVelicina ){
    
  
  const items = {
    id: proizvod.value.id,
    velicina: odabranaVelicina.value,
    boja: odabranaBoja.value,
  ...
  localStorage.setItem('basket', JSON.stringify(basket))
    ...
    console.error("Nedostaju podaci za narudžbu")
  }
};  
